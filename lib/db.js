...
async function initSql() {
  if (isVercel) {
    ...
    // Создаем функцию sql совместимую с @vercel/postgres API
    sql = {
      query: async (query, params = []) => {
        const rows = await queryFunction(query, params);
        return { rows, rowCount: rows.length };
      }
    };
  } else {
    // Локально - используем slonik для tagged template синтаксиса
    const { createPool, sql: slonikSql } = await import('slonik');
    const connectionString = process.env.POSTGRES_URL;
    ...
    sql = {
      query: async (query, params = []) => {
        const result = await pool.query(slonikSql.type(slonikSql.fragment)`${slonikSql.raw(query)}`, params);
